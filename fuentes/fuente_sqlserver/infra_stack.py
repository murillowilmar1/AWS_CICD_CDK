...
from multi_fuente_demo_cdk.constructs.lambda_function_construct import LambdaFunctionConstruct

SOURCE_NAME = "sqlserver"
SRC_DIR = Path(__file__).parent / "src"


class FuenteSqlserverInfraStack(Stack):
    """3 Glue Jobs (raw -> stage -> analytics) + 1 Lambda de auditoria
    para la fuente sqlserver.

    Consume los buckets/roles/catalogo compartidos de PlatformSharedStack;
    cada job escribe bajo el prefijo `sqlserver/` de cada bucket.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.audit_function = LambdaFunctionConstruct(
            self, "AuditFunction",
            function_name=f"{SOURCE_NAME}-audit-{env_name}",
            code_path=str(SRC_DIR / "audit"),
            handler="app.handler",
